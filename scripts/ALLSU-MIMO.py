...
def save_img_individually(img, path, start_index=0):
    ...
    for i in range(batch_size):
        ### 変更点 ###: i -> start_index + i に変更し、連番を実現
        # 例: "output/result_0.png", "output/result_1.png", ... (start_index=0 の場合)
        # 例: "output/result_20.png", "output/result_21.png", ... (start_index=20 の場合)
        individual_path = os.path.join(dirname, f"{basename}_{start_index + i}{ext}")

        # i番目の画像テンソルを取得して保存
        vutil.save_image(img[i], individual_path)

    ### 変更点 ###: ログを分かりやすく変更
    print(f"{batch_size} images are saved in {dirname}/ (starting from index {start_index})")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/T={T}"
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        help="dir to write results to",
        default=f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/nosample"
    )
    ...
    if Perfect_Estimate == True:
        opt.outdir  = f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/T={T}/perfect_estimate"
        opt.nosample_outdir = f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/nonoisenosample_perfect_estimate"
    if T == None or T <0:
        opt.outdir = f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/dynamic"
        opt.nosample_outdir = f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/dynamic/nosample"
        if Perfect_Estimate == True:
            opt.outdir = f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/dynamic_perfect_estimate"
            opt.nosample_outdir = f"outputs/ALLSU-MIMO/t={t}_r={r}_t={t}/nosample_perfect_estimate"
    ...
    ### 変更点 ###: 全画像をロード
    all_images = load_images_as_tensors(opt.input_path)
    if all_images.shape[0] == 0:
        print(f"ディレクトリ '{opt.input_path}' に画像が見つからないため、処理を終了します。")
        sys.exit()

    total_image_count = all_images.shape[0]
    print(f"合計 {total_image_count} 枚の画像をロードしました。{CHUNK_SIZE} 枚ずつのチャンクで処理します。")

    ### 追加点 ###: 保存する画像の開始インデックスを追跡するカウンター
    current_image_index = 0

    ### 追加点 ###: 全画像を CHUNK_SIZE ごとに分割してループ処理
    for img_chunk in torch.split(all_images, CHUNK_SIZE, dim=0):

        # --- ここから下の処理は、チャンクごとに行うためインデント ---

        ### 変更点 ###: バッチサイズを現在のチャンクのサイズに設定
        batch_size = img_chunk.shape[0]
        print(f"\n--- 処理中: 画像 {current_image_index + 1} から {current_image_index + batch_size} (バッチサイズ: {batch_size}) ---")

        # print(f"img shape = {img.shape}") # 元のコード
        print(f"img_chunk shape = {img_chunk.shape}")

        ### 変更点 ###: 連番保存に対応
        save_img_individually(img_chunk, opt.sentimgdir + "/sentimg.png", start_index=current_image_index)
        
        ### 変更点 ###: img -> img_chunk をデバイスに送る
        img = img_chunk.to(device=device)
        ...
        for snr in range(-5, 10, 1):
            ...
            # 正規化を元に戻す
            ### 変更点 ###: 変数名を z_restored に変更 (可読性のため)
            z_restored = q_real_data_restored * torch.sqrt(2*(z_variance + eps)).view(-1, 1, 1, 1)

            recoverd_img_no_samp = model.decode_first_stage(z_restored) # z -> z_restored
            #save_img(recoverd_img_no_samp, f"outputs/nosample_{snr}.png")
            
            ### 変更点 ###: z_restored を使用
            cond = model.get_learned_conditioning(z_restored.shape[0] * [""])
            
            print(f"####cond finisihed #####")
            
            ### 変更点 ###: z_restored を x_T として使用
            samples = sampler.MIMO_decide_starttimestep_ddim_sampling(S=opt.ddim_steps, batch_size=batch_size,
                            shape= z_restored.shape[1:4],x_T=z_restored,
            ...
            # LPIPS計算対象の 'img' は、チャンクループの先頭でデバイスに送られた 'img' (元の img_chunk)
            # print(f"LPIPS = {caluc_lpips(recoverd_img, img)}") 
            
            print(f"recoverd_img = {recoverd_img.shape}")
            
            ### 変更点 ###: 連番保存に対応
            save_img_individually(recoverd_img, f"{opt.outdir}/output_{snr}.png", start_index=current_image_index)
            save_img_individually(recoverd_img_no_samp, f"{opt.nosample_outdir}/output_{snr}.png", start_index=current_image_index)


        # --- SNRループ終了 ---

        ### 追加点 ###: 次のチャンクのために、処理した枚数分インデックスを増やす
    ...
    print(f"\n全 {total_image_count} 枚の画像の処理が完了しました。")
